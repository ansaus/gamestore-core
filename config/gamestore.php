...
return [
    ...
    */
    ...
    'reconcile' => [
        /*
        | Возраст, начиная с которого расхождение считается находкой.
        | Заказ, оплаченный секунду назад и ещё не выданный, — это не
        | проблема сверки, а нормальная асинхронность.
        */
        'grace_seconds' => (int) env('RECONCILE_GRACE_SECONDS', 60),

        // Сколько заказов трогает один проход фонового доведения.
        'batch' => (int) env('RECONCILE_BATCH', 100),

        /*
        | Потолок циклов доведения. Отличается от supplier.max_delivery_cycles:
        | тот ограничивает цепочку неопределённых исходов внутри одной выдачи,
        | этот — сколько раз watchdog вообще возьмётся за заказ, прежде чем
        | оставить его человеку (POST /api/admin/orders/{id}/retry-delivery).
        */
        'max_attempts' => (int) env('RECONCILE_MAX_ATTEMPTS', 20),

        // Потолок экспоненциального бэкоффа между циклами, секунды.
        'max_backoff_seconds' => (int) env('RECONCILE_MAX_BACKOFF', 600),
    ],
